...
const dashboard = {
    ...
    async totalPacientes (req, res){
        ...
        return res.json(`O número total de pacientes é: ${numeroPacientes.count}`);
    },

    async totalPsicologos (req, res){
        const { count } = await Psicologos.findAndCountAll();
        return res.json(`O número total de psicologos é: ${count}`);
    },

    async totalAtendimentos (req, res){
        const { count } = await Atendimentos.findAndCountAll();
        return res.json(`O número total de atendimentos é: ${count}`);
    },
    ...
    async mediaTotal (req, res){
        ...
        return res.json(`A média de att para psico é: ${numAtendimentos.count / numPiscologos.count}`);
    },
